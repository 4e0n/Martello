...
#ifndef TELLO_VID_THREAD_H
#define TELLO_VID_THREAD_H
...
class TelloVidThread : public QThread {
 Q_OBJECT
 public:
  TelloVidThread(QObject* p,TelloStruct *ts,QMutex *m,bool *t, \
		 QUdpSocket *s): QThread(p) {
   parent=p; tello=ts; mutex=m; active=t; socket=s;
  }

  virtual void run() {
   qDebug("Video Thread activated..");
   while (*active) {
    if (tello->pollConnection && \
        (socket->state()!=QAbstractSocket::ConnectedState)) {
 emit signalTelloVidConnect();
    } else if (socket->state()==QAbstractSocket::ConnectedState) {
     if (!tello->pollConnection) {
 emit signalTelloVidDisconnect();
     } else {
//      if (tello->startComm) {
...
//      }
 ...
 signals:
  void signalTelloVidConnect();
  void signalTelloVidDisconnect();
